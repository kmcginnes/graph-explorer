...
Explorer with different graph database engines and managing multiple
connections.
...
protocol, such as Blazegraph.

### Creating Connections

You can create and manage connections to graph databases using the Connections
UI. Connections is accessible as the first screen after deploying the
application, when you click `Open Connections` on the top-right. Click `+` on
the top-right to add a new connection. You can also edit and delete connections.

For detailed steps on creating your first connection, see the
[First Connection Guide](../getting-started/first-connection.md).

### Managing Multiple Connections

When you create more than one connection to a graph database, you can only
connect to and visualize from one graph database endpoint at a time. To select
the active database, toggle the "Active" switch in the Available Connections
panel.
...
- Required:
  - `PUBLIC_OR_PROXY_ENDPOINT` - `None` - See connection configuration details
- Optional
  - `GRAPH_TYPE` - `None` - If not specified, multiple connections will be
    created for every available graph type / query language
  - `USING_PROXY_SERVER` - `False` - See connection configuration details
  - `IAM` - `False` - See connection configuration details
...
- Conditionally Required:
  - Required if `USING_PROXY_SERVER=True`
    - `GRAPH_CONNECTION_URL` - `None` - See connection configuration details
  - Required if `USING_PROXY_SERVER=True` and `IAM=True`
    - `AWS_REGION` - `None` - See connection configuration details
...
For connection-related issues, see the
