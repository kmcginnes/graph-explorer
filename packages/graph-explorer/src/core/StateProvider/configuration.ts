...
/** Creates or updates an existing configuration by merging the provided config with the existing config. */
export const createOrUpdateConfigurationAtom = atom(
  null,
  async (_get, set, config: RawConfiguration) => {
    await set(configurationAtom, async prev => {
      const updated = new Map(await prev);

      const currentConfig = updated.get(config.id);
