...
import { edgeDetailsQuery } from "@/connector/queries";
...
export function EdgeSearchResult({ edge }: { edge: Edge }) {
  ...
  const detailsQuery = useQuery(
    edgeDetailsQuery(edge.__isFragment ? { edgeId: edge.id } : null, explorer)
  );

  const config = useConfiguration();
  const displayEdge = detailsQuery.data?.edge ?? edge;
  const { name, longName } = getDisplayNames(displayEdge);

  const typeConfig = config?.getEdgeTypeConfig(displayEdge.type);
  const edgeTypeDisplay =
    typeConfig?.displayLabel || textTransform(displayEdge.type);

  const addToGraph = useAddToGraph([], [displayEdge]);
  const removeFromGraph = useRemoveEdgeFromGraph(displayEdge.id);
  const hasBeenAdded = useHasEdgeBeenAddedToGraph(displayEdge.id);
  ...
  return (
    ...
    >
      ...
      <div className="border-background-secondary px-8 transition-all group-data-[expanded=false]:h-0 group-data-[expanded=true]:h-auto group-data-[expanded=true]:border-t">
        <ul>
          {Object.keys(displayEdge.attributes).map(attributeName => (
            <li
              key={attributeName}
            ...
            >
              ...
              <div className="text-text-primary">
                {displayEdge.attributes[attributeName]}
              </div>
            </li>
          ))}
        </ul>
