...
const selectedVerticesSelector = atom(async get => {
  const filteredNodeTypes = get(nodesTypesFilteredAtom);
  const allNodeTypes = await get(vertexTypesSelector);
  return new Set(
    [...allNodeTypes].filter(n => filteredNodeTypes.has(n) === false)
  );
});

const selectedEdgesSelector = atom(async get => {
  const filteredEdgeTypes = get(edgesTypesFilteredAtom);
  const allEdgeTypes = await get(edgeTypesSelector);
  return new Set(
